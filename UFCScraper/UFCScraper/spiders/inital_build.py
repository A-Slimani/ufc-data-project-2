...
class FightersSpider(scrapy.Spider):
    ...
    def parse_athletes(self, response):
        ...
        record_processed = record_raw.split(" ")[0].split("-")
        ...
        row_2col = []
        try:
            row_2col = selector.xpath(
                "//div[@class='c-bio__row--2col']/div/div[@class='c-bio__text']/text()"
            ).getall()
            if len(row_2col) == 1:
        ...
        fighter["height"] = self.inches_to_cm(bio_details["Height"]) if "Height" in bio_details else None
        fighter["reach"] = self.inches_to_cm(bio_details["Reach"]) if "Reach" in bio_details else None
        fighter["leg_reach"] = self.inches_to_cm(bio_details["Leg reach"]) if "Leg reach" in bio_details else None
        fighter["hometown"] = hometown
        fighter["trains_at"] = row_2col[0]
        fighter["fighting_style"] = row_2col[1]
